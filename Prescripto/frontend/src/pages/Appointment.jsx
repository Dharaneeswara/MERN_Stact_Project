...
const Appointment = () => {
  ...
  const getAvailableSloats = async () =>{
    setDocSlots([])
    ...
    for(let i=0; i<7;i++){
      ...
      setDocSlots(prev => ([...prev, timeSloats]))
    }
  } 
  ...
  const bookAppointment = async () =>{
    ...
    try{
      ...
      const {data} = await axios.post(backendUrl + '/api/user/book-appointment', {docId, slotDate, slotTime, userId: userData?._id,userData: userData}, {headers:{token}})
      if(data.success){
      ...
      <div className='sm:ml-72 sm:pl-4 mt-4 font-medium text-gray-700'>
        ...
        <div className='flex items-center gap-3 w-full overflow-x-scroll mt-4'>
          {docSlots.length && docSlots[slotIndex].map((item, index)=>(
            <p onClick={()=>setSloatTime(item.time)} className={`text-sm font-light flex-shrink-0 px-5 py-2 rounded-full cursor-pointer ${item.time === slotTime ? 'bg-[#5f6FFF] text-white' : 'text-gray-400 border border-gray-300'}`} key={index}>
